...
const CATEGORY_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  architecture:   { bg: "rgba(59,130,246,0.1)",   text: "var(--primary-dim)",  border: "rgba(59,130,246,0.2)"  },
  implementation: { bg: "rgba(34,197,94,0.08)",   text: "#4ade80",            border: "rgba(34,197,94,0.2)"   },
  debugging:      { bg: "rgba(245,158,11,0.1)",   text: "#fbbf24",            border: "rgba(245,158,11,0.2)"  },
  onboarding:     { bg: "rgba(139,92,246,0.1)",   text: "var(--secondary-dim)",border: "rgba(139,92,246,0.2)" },
};

const CATEGORY_DEFAULT = { bg: "var(--surface-2)", text: "var(--text-muted)", border: "var(--border)" };

const STACK_COLORS: Record<string, { gradient: string; shadow: string }> = {
  Python: { gradient: "linear-gradient(155deg, #60a5fa 0%, #3b82f6 100%)", shadow: "0 4px 12px rgba(59, 130, 246, 0.28)" }, // blue
  TypeScript: { gradient: "linear-gradient(155deg, #818cf8 0%, #6366f1 100%)", shadow: "0 4px 12px rgba(99, 102, 241, 0.28)" }, // indigo
  JavaScript: { gradient: "linear-gradient(155deg, #818cf8 0%, #6366f1 100%)", shadow: "0 4px 12px rgba(99, 102, 241, 0.28)" }, // indigo
  Go: { gradient: "linear-gradient(155deg, #22d3ee 0%, #06b6d4 100%)", shadow: "0 4px 12px rgba(6, 182, 212, 0.28)" }, // cyan
  Rust: { gradient: "linear-gradient(155deg, #fb923c 0%, #f97316 100%)", shadow: "0 4px 12px rgba(249, 115, 22, 0.28)" }, // orange
  Java: { gradient: "linear-gradient(155deg, #f87171 0%, #ef4444 100%)", shadow: "0 4px 12px rgba(239, 68, 68, 0.28)" }, // red
  Ruby: { gradient: "linear-gradient(155deg, #fb7185 0%, #f43f5e 100%)", shadow: "0 4px 12px rgba(244, 63, 94, 0.28)" }, // rose
};

const DEFAULT_STACK_COLOR = {
  gradient: "linear-gradient(155deg, #6c6cdf 0%, #4d4dc9 100%)",
  shadow: "0 4px 12px rgba(91,91,214,0.28)",
};
...
// ── Main RepoIdentityCard ───────────────────────────────────────────────────

export interface RepoIdentityCardProps {
  persona: RepoPersona;
  repoName?: string;
...
export default function RepoIdentityCard({
  persona,
  repoName,
  onQuestionSelect,
  onRegenerate,
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{
                background: (STACK_COLORS[persona.dominant_language] || DEFAULT_STACK_COLOR).gradient,
                boxShadow: (STACK_COLORS[persona.dominant_language] || DEFAULT_STACK_COLOR).shadow,
              }}
            >
...
export function RepoIdentityCardSkeleton() {
  return (
